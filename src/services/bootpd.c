...
#include "libs/ping_api/ping_api.h"
#include "threading.h"
        ...
        if (pCurIP == NULL || !sSettings.bPing || nDhcpType != DHCPDISCOVER)
            ...
            if (FindNearestServerAddress((struct in_addr*)&pCurIP->dwIP, (struct in_addr*)&mask, TRUE) != NULL) {
                ...
                if (Rc > 0) {
                    ...
                    LOG(1, "Ping Error %d (%s)", WSAGetLastError(), LastErrorText());
                    Sleep(100);
                    bFound = TRUE;
                }
    ...
    *(DWORD*)(pDhcpPkt->options) = *(DWORD*)DHCP_OPTIONS_COOKIE;
    ...
    for (Ark = 0; Ark < SizeOfTab(sDhcpOpt); Ark++) {
        ...
        switch (sDhcpOpt[Ark].nDHCPOpt) {
            case DHO_DHCP_MESSAGE_TYPE: *pOpt = (unsigned char)nDhcpType;
                break ;
            case DHO_LOG_SERVERS: // fallthrough
            case DHO_DHCP_SERVER_IDENTIFIER: *(DWORD*)pOpt = pNearest->s_addr;
                break ;
            case DHO_TFTP_SERVER:
                ...
                *pOpt = lstrlen(sSettings.szTftpLocalIP[0] != 0 ? sSettings.szTftpLocalIP : inet_ntoa(*pNearest));
                memcpy(pOpt + 1, sSettings.szTftpLocalIP[0] != 0 ? sSettings.szTftpLocalIP : inet_ntoa(*pNearest),
                       *pOpt);
                pOpt += 1 + *pOpt;
                break;

            case DHO_SUBNET_MASK: *(DWORD*)pOpt = inet_addr(sParamDHCP.szMask);
                break ;
            ...
            case DHO_NETBIOS_NAME_SERVERS:
                ...
                break;
            case DHO_DHCP_LEASE_TIME: *(DWORD*)pOpt = htonl(sParamDHCP.nLease * 60);
                break ;
            case DHO_DHCP_RENEWAL_TIME: *(DWORD*)pOpt = htonl(sParamDHCP.nLease / 2 * 60);
                break ;
            case DHO_DHCP_REBINDING_TIME: *(DWORD*)pOpt = htonl((sParamDHCP.nLease * 80) / 100 * 60);
                break ;
            case DHO_BOOT_SIZE:
        ...
        case DHCPREQUEST: {
            ...
            if (bSERVER || (pDhcpPkt->ciaddr.S_un.S_addr == 0 && sSettings.bPXECompatibility)) {
                ...
                *pSize = DHCPOptionsReply(pDhcpPkt, DHCPACK, receivingAddress, iLastArch);
                ...
                LOG(
                    5,
                    "Client requested address %s which was not allocated by tftpd32 and is either outside our pool or is used by someone else",
