...
tThreadsConfig[] =
{
    ...
    "DHCP", TFTPD32_DHCP_SERVER, ListenDhcpMessage, FALSE, 8192, AF_INET, SOCK_DGRAM, "bootps", &BootPdPort,
    BOOTPD_PORT,NULL, FALSE, TRUE, TRUE,
    "TFTP", TFTPD32_TFTP_SERVER, TftpdMain, FALSE, 4096, AF_UNSPEC, 0, "tftp", &sSettings.Port, TFTP_PORT,
    sSettings.szTftpLocalIP, TRUE, TRUE, TRUE,
    "SNTP", TFTPD32_SNTP_SERVER, SntpdProc, FALSE, 4096, AF_UNSPEC, SOCK_DGRAM, "ntp", &SntpdPort, SNTP_PORT, "", FALSE,
    FALSE, TRUE,
    "DNS", TFTPD32_DNS_SERVER, ListenDNSMessage, FALSE, 4096, AF_UNSPEC, SOCK_DGRAM, "domain", &DnsPort, DNS_PORT, NULL,
    FALSE, FALSE, TRUE,
    "Syslog", TFTPD32_SYSLOG_SERVER, SyslogProc, FALSE, 4096, AF_UNSPEC, SOCK_DGRAM, "syslog", &SyslogPort, SYSLOG_PORT,
    "", FALSE, FALSE, TRUE,
};
...
static int FakeServiceMessage(const char* name, int family, int type, const char* service, int def_port,
                              const char* sz_if) {
    SOCKET s;
    int Rc;
    ADDRINFO Hints, *res;
...
int WakeUpThread(int Idx) {
    ...
    if (tThreadsConfig[Idx].wake_up_by_ev) {
        ...
        FakeServiceMessage(tThreadsConfig[Idx].name,
