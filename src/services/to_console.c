    ...
    switch (pmsg->type) {
        case C_CONS_KILL_TRF:
            ...
            for (pTftp = pTftpFirst;
                 ...
                 pTftp = pTftp->next);
            if (pTftp != NULL) {
                nak(pTftp, ECANCELLED);
                pTftp->st.ret_code = TFTP_TRF_STOPPED;
            }
            break;
...
void TftpdConsole(void* param) {
    ...
    do {
        ...
        WSACloseEvent(tEvents[EV_FROMGUI]);
        tEvents[EV_FROMGUI] = INVALID_HANDLE_VALUE;
        closesocket(sDlg);

        // blocking service not available
        CloseHandle(hEvMsgSent);
        hEvMsgSent = INVALID_HANDLE_VALUE;
        CloseHandle(hMsgRequestSemaf);
        hMsgRequestSemaf = INVALID_HANDLE_VALUE;
