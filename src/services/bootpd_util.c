                ...
                for (nLen = 0; *p != 0 && nLen < len - 8; p = q) {
                    ...
                    switch (cvt[min(q-p, 8)]) {
                        case 1: sscanf(p, "%x", &c);
                            buffer[nLen] = (char)c;
                            break;
                        case 2: sscanf(p, "%hx", buffer + nLen);
                            break;
                        case 4: sscanf(p, "%lx", buffer + nLen);
                            break;
                        case 8: sscanf(p, "%llx", buffer + nLen);
                            break;
                    }
                    nLen += cvt[min(q-p, 8)];
...
char* TranslateExp(const char* exp, char* to, struct in_addr ip, const char* tMac, unsigned short my_iLastArch) {
    ...
    // truncate input
    Ark = strnlen(to, DHCP_FILE_LEN - 1);
    to[Ark] = 0;
...
void IncNumAllocated() {
    ...
    if (sSettings.bPersLeases)
        AsyncSaveKey(TFTPD32_DHCP_KEY, KEY_LEASE_NUMLEASES, &nAllocatedIP, sizeof(nAllocatedIP), REG_DWORD,
                     szTftpd32IniFile);
}
