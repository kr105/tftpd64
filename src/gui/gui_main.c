...
enum {
    IDM_TFTP_HIDE = 0x1000,
    IDM_TFTP_EXPLORER,
    IDM_STOP_SERVICES,
    IDM_START_SERVICES,
    IDM_RESTART_SERVICES
}; // should be under 0xF000
...
int ChangeIPAddress(HWND hWnd, struct S_IPAddressList* pIf) {
    ...
    switch (nService) {
        case TFTPD32_TFTP_SERVER: szFind = sGuiSettings.szTftpLocalIP;
            break;
        case TFTPD32_DHCP_SERVER: szFind = sGuiSettings.szDHCPLocalIP;
            break;
        default: szFind = NULL;
    }
...
int WndDrawInterfacesComboBox(HWND hWnd, WPARAM wParam, LPARAM lParam) {
    ...
    ExtTextOut(lpdis->hDC, x * sizeof ("255.255.255.255"), y, ETO_CLIPPED, &lpdis->rcItem, pif->descr,
               lstrlen(pif->descr), NULL);
    return 0;
    ...
    switch (message) {
        ...
        case WM_NOTIFYTASKTRAY:
            switch (lParam) {
                ...
                case WM_RBUTTONDOWN:
                    ...
                    if (Rc != 0)
                        PostMessage(hWnd, WM_SYSCOMMAND, Rc, 0);
                    break;
        ...
        */
        ...
        case WM_SYSCOMMAND:
            switch (wParam) {
                case IDM_TFTP_HIDE:
                    CheckMenuItem(GetSystemMenu(hWnd, FALSE), IDM_TFTP_HIDE,
                                  IsWindowVisible(hWnd) ? MF_CHECKED : MF_UNCHECKED);
...
void ReturnToPreviousInstance(HANDLE myMutEx, LPCSTR lpszCmdLine) {
    ...
    if (strstr(lpszCmdLine, "-kill") != NULL) {
        ...
        if (IsWindow(hPrevWnd)) {
            Sleep(1000);
            PostMessage(hPrevWnd, WM_DESTROY, 0, 0);
        }
