...
///////////////////////////////////
// positionne la Gauge Window dans la fen�tre TFTP
///////////////////////////////////
static int NewGaugePos(HWND hWnd, int n) {
    ...
    // Ajustement des param�tres (barres de t�ches et non recouvrement)
    sGaugeClientRect.bottom += 10;
    sGaugeClientRect.right += 0;
    sTftpRect.top += sTftpRect.bottom - sTftpRect.top - sTftpClientRect.bottom;
    sTftpRect.left += 10;

    // Nombre de fen�tres en vertical et Horizontal.
    // Attention, on peut avoir sTftpClientRect = {0,0}.
    // Pour �viter la division par 0, on force NbVert=3, NbHoriz=1.
