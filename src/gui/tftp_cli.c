...
int BadEndOfTransfer(const char* szFmt, ...) {
    ...
    if (sTC.opcode == TFTP_RRQ && sTC.nRcvd != 0)
        DeleteFile(sTC.szFile);
    return FALSE;
} // BadEndOfTransfer
...
static void PopulateXRQPacket(char* szBlkSize, BOOL bFullPath) {
    ...
    if (isdigit(szBlkSize[0])) {
        ...
        lstrcpy(&sTC.BufSnd[sTC.nToSend], szBlkSize);
        sTC.nToSend += lstrlen(szBlkSize) + 1;
    }
    if (sGuiSettings.bPortOption) {
        lstrcpy(&sTC.BufSnd[sTC.nToSend], TFTP_OPT_PORT), sTC.nToSend += sizeof TFTP_OPT_PORT;
        lstrcpy(&sTC.BufSnd[sTC.nToSend], "0");
        sTC.nToSend += sizeof "0";
    }
...
void static Statistics(time_t dNow) {
    ...
    if (dNow != sTC.dLastUpdate) {
        ...
        if (sTC.dwFileSize / 100 != 0)
            SendDlgItemMessage(GetParent(hTftpClientWnd), IDC_CLIENT_PROGRESS, PBM_SETPOS,
                               (sTC.nCount * sTC.nPktSize) / (sTC.dwFileSize / 100), 0);
    } // time to update
}     // Statistics
    ...
    switch (wItem) {
        ...
        // datagram size. May be modified by an OACK message
            ...
            if (sTC.szHost[0] == 0 || sTC.szFile[0] == 0)
                ...
                Rc = WSAAsyncSelect(sTC.s, hWnd, wItem == IDC_CLIENT_GET_BUTTON ? WM_CLIENT_DATA : WM_CLIENT_ACK,
                                    FD_READ);
                Rc = GetLastError();
...
LRESULT CALLBACK TftpClientProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    switch (message) {
        ...
        // fEditBoxProc = (WNDPROC) SetWindowLong (GetDlgItem (hParentWnd, IDC_CLIENT_LOCALFILE), GWL_WNDPROC, (LONG) TftpClientFileNameProc);
            fEditBoxProc = (WNDPROC)SetWindowLongPtr(GetDlgItem(hParentWnd, IDC_CLIENT_LOCALFILE), GWLP_WNDPROC,
                                                     (LONG_PTR)TftpClientFileNameProc);
        //    SetWindowLong (GetDlgItem (hParentWnd, IDC_CLIENT_FILE), GWL_USERDATA, (LONG) TftpClientFileNameProc);
            SetWindowLongPtr(GetDlgItem(hParentWnd, IDC_CLIENT_LOCALFILE), GWLP_USERDATA,
                             (LONG_PTR)TftpClientFileNameProc);
            break;
        ...
        // On est reveill� par un message re�u
            ...
            }     // il y a un message � recevoir
        // La comparaison marche si le paquet est le bon ou une r�p�tition du pr�c�dent message
        ...
        // sTC.nRcvd ne peut �tre inf�rieur que si on a re�u un block
        ...
        // socket not closed by StopTransfer
            ...
            // Timeout or ack of previous block
                ...
                // une possibilit� : on est au dernier message et le serveur TFTP n'a pas acquitt�
