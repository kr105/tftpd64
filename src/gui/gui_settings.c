...
static int ReadNewSettings(HWND hWnd) {
    ...
    if (ISDLG_CHECKED(hWnd, IDC_CHECK_LOCALIP_DHCP)) {
        ...
        ComboBox_GetText(GetDlgItem(hWnd, IDC_CB_LOCALIP_DHCP), sNewSettings.szDHCPLocalIP,
                         sizeof sNewSettings.szDHCPLocalIP);
    ...
    if (IsGuiConnectedToRemoteService())
        ...
        if (IsValidDirectory(sNewSettings.szBaseDirectory)) {
            ...
            MY_WARNING("TFTP: Can not change directory");
    }
...
BOOL Tftpd32SaveWindowPos(HWND hMainWnd) {
    ...
    Rc = RegCreateKeyEx(
        ...
        &dwState);
    if (Rc == ERROR_SUCCESS)
        SAVEKEY(KEY_WINDOW_POS, sz, REG_SZ);
...
BOOL Tftpd32RetrieveWindowPos(HWND hMainWnd) {
    ...
    // Lire les donn�es dans cette entr�e
    ...
    for (sz[sizeof sz - 1] = 0, pNext = pCur = sz; *pNext != 0; pNext++)
        if (*pNext == ' ') {
            *pNext = 0;
            switch (Ark++) {
                case 0: R.left = atoi(pCur);
                    break;
                case 1: R.top = atoi(pCur);
                    break;
                case 2: R.right = atoi(pCur);
                    break;
                case 3: R.bottom = atoi(pCur);
                    break;
            }
            pCur = pNext + 1;
        }
