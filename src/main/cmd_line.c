...
int ProcessCmdLine(LPSTR lpszCmdLine) {
    ...
    for (Ark = 1; Ark < argc; Ark++) {
        if (argv[Ark][0] == '-' && argv[Ark + 1] != NULL) {
            switch (argv[Ark][1]) {
                case 's': SetEnvironmentVariable(TFTP_DIR, argv[++Ark]);
                    break;
                case 'l': SetEnvironmentVariable(TFTP_LOG, argv[++Ark]);
                    break;
                case 'i': SetEnvironmentVariable(TFTP_INI, argv[++Ark]);
                    break;
#ifdef SERVICE_EDITION
                case 'h': SetEnvironmentVariable(TFTP_HOST, argv[++Ark]);
                    break;
                case 'p': SetEnvironmentVariable(TFTP_PWD, argv[++Ark]);
                    break;
#endif
            }
        }
